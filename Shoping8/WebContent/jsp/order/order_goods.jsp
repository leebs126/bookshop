...
function fn_show_order_detail(){
	...
	var h_order_total_price=document.getElementById("h_order_total_price");
	...
	var p_order_total_price=document.getElementById("p_order_total_price");
	...
	p_order_goods_qty.innerHTML=order_goods_qty+"개";
	p_order_total_price.innerHTML=order_total_price+"원";
...
function fn_process_pay_order(){
    ...
    formObj.method="post";
    formObj.action="/Shoping8/order/order.do";
    formObj.submit();
/* 	
	alert("hp: "+hp);
				...
				<c:forEach var="item" items="${my_order_list }">
					...
					<td>
					  <h2>${item.goods_qty }개<h2>
					    <input   type="hidden" id="h_order_goods_qty" name="h_order_goods_qty" value="${item.goods_qty}" />
					</td>
					<td><h2>${item.goods_sales_price * item.goods_qty}원 (10% 할인)</h2></td>
					<td><h2>0원</h2></td>
					<td><h2>${1500 *item.goods_qty }원</h2></td>
					<td>
					  <h2>${item.goods_sales_price * item.goods_qty}원</h2>
					  <input  type="hidden" id="h_order_total_price"  name="h_order_total_price" value="${item.goods_sales_price * item.goods_qty}" />
					</td>
			</TR>
			<c:set var="final_total_order_price"
				value="${final_total_order_price+ item.goods_sales_price* item.goods_qty}" />
			<c:set var="total_order_price"
				value="${total_order_price+ item.goods_sales_price* item.goods_qty}" />
			<c:set var="total_order_goods_qty"
				value="${total_order_goods_qty+item.goods_qty }" />
			</c:forEach>
		</TBODY>
	</TABLE>
	<DIV class="clear"></DIV>
			...
			<TBODY>
				...
				<TR class="dot_line">
					...
					<TD><input id="delivery_message" name="delivery_message" type="text" size="50"
						value="택배 기사님께 전달할 메시지를 남겨주세요." /></TD>
				</TR>
				<TR class="dot_line">
		...
		<TBODY>
			...
			<tr cellpadding=40 align=center>
				<td id="">
					...
					id="h_totalNum" type="hidden" value="${total_order_goods_qty}" />
				</td>
				<td>
				...
				<td>
					<p id="p_totalSalesPrice">${total_sales_price }원</p> <input
					id="h_totalSalesPrice" type="hidden" value="${total_sales_price}" />
				</td>
				<td><IMG width="25" alt=""
					src="<%=request.getContextPath()%>/image/equal.jpg"></td>
				<td>
					<p id="p_final_totalPrice">
						<font size="15">${final_total_order_price }원 </font>
					</p> <input id="h_final_totalPrice" type="hidden"
					value="${totalPrice+totalDelivery-totalSalesPrice  }" />
				</td>
			</tr>
		</TBODY>
	</table>
			...
			<table >
				<TBODY align=left>
				   ...
				   <tr>
					 ...
					 <td >
					      <p id="p_order_total_price">주문금액합계</p>
					 </td>
				   </tr>
					<tr>
					  <td width=200px>
