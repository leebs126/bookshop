...
function fn_process_pay_order(){
    ...
    */
    ...
    formObj.method="post";
    formObj.action="/Shoping11/order/order.do";
    formObj.submit();
/* 	
	alert("hp: "+hp);
				...
				<c:forEach var="item" items="${my_order_list }">
					<TD class="goods_image">
					  <a href="<%=request.getContextPath()%>/goods/goods.do?command=goods_detail&goods_id=${item.goods_id }">
					    <IMG width="75" alt=""  src="<%=request.getContextPath() %>/download.do?goods_id=${item.goods_id}&fileName=${item.fileName}"">
					    <input   type="hidden" id="h_goods_id" name="h_goods_id" value="${item.goods_id }" />
					    <input   type="hidden" id="h_goods_fileName" name="h_goods_fileName" value="${item.fileName }" />
					  </a>
					</TD>
					<TD>
					  <h2>
					...
					<td><h2>${item.goods_sales_price * item.order_goods_qty}원 (10% 할인)</h2></td>
					<td><h2>0원</h2></td>
	...
	<table width=80% class="list_view" style="background: #ccffff">
		<TBODY>
			...
			<tr cellpadding=40 align=center>
				...
				<td>
					<p id="p_totalSalesPrice">${total_sales_price }원</p> 
